...
TaskScheduler provides a means to run rake tasks on a schedule triggered by a simple cron.
...
In your Rails application, add a route
    mount TaskScheduler::Engine => "/"
The url will be http://0.0.0.0:3000/scheduled_tasks

If your route is
    mount TaskScheduler::Engine => "/ts"
then the url will be http://0.0.0.0:3000/ts/scheduled_tasks
...
# /home/rails/cron_jobs/task_scheduler_cron.sh
    ...
    /home/rails/.rvm/gems/ruby-1.9.2-p180@rails2/bin/rake RAILS_ENV=production task_scheduler:run

_It's always a good idea to use full paths in scripts for cron._

=== Add a line to your crontab
    1 * * * * /home/rails/cron_jobs/task_scheduler_cron.sh
